...
function endTagOpen(c) {
    if (c.match(/^[a-zA-Z]$/)) {
        ...
        throw Error(`unexcept error charactor "${c}"`)
    } else if (c === EOF) {
        throw Error(`unexcept error charactor "${c}"`)
    }
}
...
function attributeName(c) {
    if (c.match(/^[\t\n\f\s]$/) || c === '/' || c === '>' || c === EOF) {//一个属性,没有值，收集完成（比如disabled,readonly）
        currentToken[currentAttribute.name] = true;//给currentToken添加一个属性
        return afterAttributeName(c);
    } else if (c === '=') {
